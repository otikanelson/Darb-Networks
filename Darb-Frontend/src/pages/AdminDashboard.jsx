...
const AdminDashboard = () => {
  ...
  // Check admin access
  useEffect(() => {
    console.log('🔐 Checking admin access:', { user, isAuthenticated: isAuthenticated() });
  ...
  const handleApproveCampaign = async (campaignId, comments = '', isFeatured = false) => {
    try {
      console.log(`✅ Approving campaign ${campaignId}`);
      const token = localStorage.getItem('authToken');
      const response = await fetch(`http://localhost:5000/api/admin/campaigns/${campaignId}/approve`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleRejectCampaign = async (campaignId, comments) => {
    ...
    try {
      ...
      const token = localStorage.getItem('authToken');
      const response = await fetch(`http://localhost:5000/api/admin/campaigns/${campaignId}/reject`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-600">
                    {(stats.userCounts?.founder || 0) + (stats.userCounts?.investor || 0)}
                  </p>
                </div>
            ...
            {campaignsLoading ? (
              ...
              <div className="text-center py-12">
                ...
                <div className="mt-4 text-xs text-gray-400">
                  Current filter: {statusFilter} | Campaigns loaded: {campaigns.length}
                </div>
              </div>
            ) : (
              <div className="space-y-4">
        ...
        <div className="flex justify-between items-start mb-4">
          <div className="flex-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-600">
              <div>
                <span className="font-medium">Founder:</span> {campaign.founder_name}
                {campaign.founder_company && (
                  <span className="block text-gray-500">{campaign.founder_company}</span>
                )}
              </div>
              <div>
              ...
              <div>
                <span className="font-medium">Target:</span> {formatCurrency(campaign.target_amount)}
                <span className="block text-gray-500">Min: {formatCurrency(campaign.minimum_investment)}</span>
              </div>
            </div>

            <div className="mt-3 text-sm text-gray-600">
              <div className="flex items-center gap-4">
                <span><Eye className="h-4 w-4 inline mr-1" />{campaign.view_count || 0} views</span>
            ...
            {campaign.admin_comments && (
              <div className="mt-3 p-3 bg-gray-50 rounded-lg">
                ...
                <p className="text-sm text-gray-600 mt-1">{campaign.admin_comments}</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="mt-6 pt-6 border-t border-gray-200 space-y-4">
            ...
            {campaign.problem_statement && (
              <div>
                ...
                <p className="text-gray-600 text-sm">{campaign.problem_statement}</p>
              </div>
            )}
            ...
            {campaign.business_plan && (
              <div>
                <h4 className="font-medium text-gray-900 mb-2">Business Plan</h4>
                <p className="text-gray-600 text-sm">{campaign.business_plan}</p>
              </div>
            )}

            {campaign.main_image_url && (
              <div>
                <h4 className="font-medium text-gray-900 mb-2">Campaign Image</h4>
                <img 
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                  <span className="font-medium">Email:</span> {campaign.founder_email}
                </div>
                {campaign.founder_phone && (
                  <div>
                    <span className="font-medium">Phone:</span> {campaign.founder_phone}
                  </div>
                )}
                {campaign.founder_company && (
                  <div>
                    <span className="font-medium">Company:</span> {campaign.founder_company}
                  </div>
                )}
                {campaign.founder_address && (
                  <div>
                    <span className="font-medium">Address:</span> {campaign.founder_address}
                  </div>
                )}
              </div>
            </div>
      ...
      {showRejectModal && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Reject Campaign</h3>
              <p className="text-sm text-gray-600 mb-4">
                Please provide a reason for rejecting this campaign. This will be shared with the founder.
              </p>
